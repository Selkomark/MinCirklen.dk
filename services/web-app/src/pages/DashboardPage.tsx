...
const COMMUNITY_RULES = [
  ...
  },
  {
    ...
    description:
      "Don't promote a business, service, or product, and don't ask other participants to pay for anything.",
  },
  {
    icon: '⚠️',
...
function ReportSessionModal({
  ...
  const reportable = PARTICIPANTS.filter((p) => p.id !== 'you')
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            <DangerButton
              ...
              onClick={() => {
                const about = PARTICIPANTS.filter((p) => aboutIds.includes(p.id))
                  .map((p) => p.label)
                  .join(', ')
